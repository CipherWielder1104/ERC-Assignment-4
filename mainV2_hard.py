...
while True:
    ...
    if display_screen:
        # Create a screen for choosing 'n' or 'h'
        cv2.putText(frame , "Press 'n' to play the game in normal mode  ", (10, 200),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(frame , "Press 'h' to play the game in hard mode  ", (10, 250),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(frame , "Press 'q' anytime to quit the game/screen  ", (10, 300),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.imshow('Choose Mode', frame)

        if pressedKey == ord('n'): #type n for normal mode 
            display_screen = False
            game_screen_hard = False
            game_screen_normal = True
            ...
            def is_within_acceptance(puck, target, acceptance_percent=5):
                ...
                for i in range (0,tcords):
                    ...
                    acceptance_distance = 25 + 25* (acceptance_percent / 100.0)
        ...
        if pressedKey == ord('h'):  #type h for harder mode
            ...
            game_screen_normal = False
            target_i=cv2.resize(point_img , (10,10))
            ...
            # Calculate the distance between the puck and the target
                for i in range (0,tcords):
                    distance = (((puck[0] - target[0]) ** 2) + ((puck[1] - target[1]) ** 2)) ** 0.5
                    # Calculate the acceptance distance
                    acceptance_distance = 10 + 10 * (acceptance_percent / 100.0)
                    # Check if the puck is within the acceptance area
                if distance <= acceptance_distance:
                        return True
                else:
        ...
        if not gameover():
            ...
            # Check for collisions with the paddle
            if (
            paddle_position[1] - 15 <= puck_pos[1] <= paddle_position[1] + 15
            and paddle_position[0] - 15 <= puck_pos[0] <= paddle_position[0] + 15):
                puck_velocity[1] *= -1
            ...
            if results.multi_hand_landmarks and results.multi_hand_landmarks[0].landmark:
                paddle_center = (paddle_position[0], paddle_position[1])
                cv2.circle(frame, paddle_center , 15 ,(0,255,0), cv2.FILLED)
        ...
        if pressedKey == ord('q'): # Exit the game when 'q' is pressed
            ...
            game_screen_normal = False 
            break
    elif  pressedKey == ord('q'): # Exit the game when 'q' is pressed
        break
